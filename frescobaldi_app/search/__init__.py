...
class Search(plugin.MainWindowPlugin, QWidget):
    def __init__(self, mainwindow):
        ...
        self.prevButton = QToolButton(autoRaise=True, focusPolicy=Qt.FocusPolicy.NoFocus, clicked=self.findPrevious)
        self.prevButton.setIcon(icons.get('go-previous'))
        self.nextButton = QToolButton(autoRaise=True, focusPolicy=Qt.FocusPolicy.NoFocus, clicked=self.findNext)
        ...
        self.hideAction.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogCloseButton))
    ...
    def event(self, ev):
        ...
        if ev.type() == QEvent.Type.ShortcutOverride and ev.matches(QKeySequence.StandardKey.HelpContents):
